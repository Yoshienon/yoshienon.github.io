class AudioController {
    constructor() {
        this.bgMusic = new Audio('Assets/Audio/Disney.mp3');
...
class MatchingCardGame {
    ...
    const shuffled = Array.from(cardsArray);
    for (let i = shuffled.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [shuffled[i], shuffled[j]] = [shuffled[j], shuffled[i]];
    }
    const container = shuffled[0].parentNode;

    const fragment = document.createDocumentFragment();
    shuffled.forEach(card => fragment.appendChild(card));
    container.appendChild(fragment);

    this.cardsArray = shuffled;
    }
...
function ready() {
    ...
    let game = new MatchingCardGame(100, cards);
